...
def execute_go(exits, direction):
...
def execute_take(item_id, inventory, room):
    ...
    for item in current_room["items"]:
        if item_id != item:
            new.append(item)
    if len(new) == len(current_room["items"]):
...
def execute_drop(item_id, inventory, room):
    ...
    for item in inventory:
        if item != item_id:
            inventoryTemp.append(item)
    if len(inventory) == len(inventoryTemp):
